...
result.with.error
@
The result is \Sexpr{round(result.with.error,6)}. Note that I did not type the result, but I used the \verb|Sexpr{}| command.
\subsection{Plotting}
We can also plot with R.
<<Hist, eval=FALSE, size="scriptsize", warning=FALSE, message=FALSE>>=
...
\end{table}
  ...
  The \texttt{palmerpenguins} package for R \citep{palmerpenguins} provides data on adult foraging penguins near Palmer Station,
Antarctica. Figure \ref{Figure 2} is too big to fit nicely in our column-based-template above, so I’ve placed it here in the abstract by
saving it and presenting it scaled to 0.75.



<<results='hide', size="scriptsize">>=
